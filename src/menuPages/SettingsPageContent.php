...
function displayTabNavigation($currentTab) {
    ...
    <nav class="nav-tab-wrapper">
        <a href="?page=ei_settings-page&tab=default" class="nav-tab <?php echo $currentTab === "default" ? "nav-tab-active" : ""; ?>">
            <?php echo esc_html__("General", "easyicon"); ?>
        </a>
        <a href="?page=ei_settings-page&tab=fontselect" class="nav-tab <?php echo $currentTab === "fontselect" ? "nav-tab-active" : ""; ?>">
...
function displayGeneralTab() {
    ?>
    <h2><?php echo esc_html__("EasyIcon", "easyicon"); ?></h2>
...
function displayFontSelectionForm() {
    ...
    if (!empty($available_fonts)) {
        ...
        echo '<p>' . esc_html__("No available fonts found. Please upload font files.", "easyicon") . '</p>';
    }

    echo '<p><input type="submit" class="button button-primary" value="' . esc_html__("Save Font Selection", "easyicon") . '"></p>';
}
        ...
        if (wp_verify_nonce($remove_font_nonce, 'remove_easyicon_font')) {
            ...
            if (!empty($font_to_remove)) {
                ...
                $message = $remove_result 
                    ? esc_html__("Font removed successfully.", "easyicon")
                    : esc_html__("Failed to remove the font.", "easyicon");
    ...
    if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['custom_font']) && isset($_POST['upload_custom_font_nonce'])) {
        ...
        if (wp_verify_nonce($upload_custom_font_nonce, 'upload_custom_font')) {
            ...
            if (!in_array(strtolower($file_extension), $valid_extensions)) {
                ...
                $message = $font_added 
                    ? esc_html__("Font uploaded and added successfully.", "easyicon")
                    : esc_html__("Failed to add the font.", "easyicon");
